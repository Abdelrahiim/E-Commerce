...
def test_promotion_price_reduction(
    reduction, result, celery_app, celery_worker, promotion_multi_variant
):
    promotion_prices(reduction_amount=reduction, obj_id=promotion_multi_variant.id)
    new_price = Promotion.product_on_promotion.through.objects.get(
