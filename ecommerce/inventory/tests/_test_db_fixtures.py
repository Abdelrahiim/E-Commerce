...
def test_inventory_db_product_uniqueness_integrity(db, product_factory):
    web_id = product_factory.create(web_id=12343434343)
    with pytest.raises(IntegrityError):
        product_factory.create(web_id=12343434343)
...
def test_inventory_db_product_insert_data(
    ...
    new_product = product_factory.create(category=(1, 12, 24, 35))
    result_product_category = new_product.category.all()
    # print(result_product_category)
    assert "web_id_" in new_product.web_id  # Test If The Web id is correct
    assert (
        result_product_category.count() == 4
    )  # Test if The The Product has 2 Cats
...
def test_inventory_db_producttype_insert_data(db, product_type_factory):
    new_type = product_type_factory.create(name="demo")
    assert new_type.name == "demo"


# ---------------------------------------------------------------
def test_inventory_db_producttype_name_uniqueness(db, product_type_factory):
    new_type = product_type_factory.create(name="Gag")
    with pytest.raises(IntegrityError):
        product_type_factory.create(name="Gag")


# --------------------------------------------------------------
def test_inventory_db_brand_insert_data(db, brand_factory):
    new_brand = brand_factory.create(name="new_brnad")
    assert new_brand.name == "new_brnad"


# --------------------------------------------------------------
def test_inventory_db_producttype_name_uniqueness(db, brand_factory):
    new_type = brand_factory.create(name="Gag")
    with pytest.raises(IntegrityError):
        brand_factory.create(name="Gag")
...
# test if the fixture is loaded into the test database
...
# ---------------------------------------------------------------
# test if we can insert new data into the test database after loading the fixtures
...
# ---------------------------------------------------------------
def test_inventory_db_product_attribute_uniqueness(
    db, product_attribute_factory
):
    new_attribute = product_attribute_factory.create(name="not_unique")
    with pytest.raises(IntegrityError):
