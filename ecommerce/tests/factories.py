...
class CategoryFactory(factory.django.DjangoModelFactory):
    ...
    # Generate The Same name EveryTime
    # name = fake.lexify(
    #     text="cat_name_??????????"
    # )  # generter differnet name in the place of the ?? every time it called
...
class ProductFactory(factory.django.DjangoModelFactory):
    ...
    slug = fake.lexify(text="product_name_???????")
    name = fake.lexify(text="product_name_???????")
    description = fake.text()
    is_active = True
    created_at = "2023-03-04 22:14:18.279092"
    updated_at = "2023-03-04 22:14:18.279092"
